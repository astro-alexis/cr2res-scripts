...
data['spec_master'] = np.zeros((nord,npix))
...
data['fit_wave_offset'] = np.zeros((nord,nobs))
data['fit_scaling'] = np.zeros((nord,nobs))
...
for i in range(len(rawf)):
    ...
    for o in range(len(red)-1):
        ...
        data['fit_scaling'][index,i],data['fit_wave_offset'][index,i] = res.x[0], res.x[1]
        ...
        data['spec_master'][index,:] = refflux
        index+=1
